...
void AIAlgorithm::Pathfind(AActor* Actor, FVector Destination, UWorld* Worldz){
    ...
    while(OpenVectors.Num() != 0){
        ...
        UE_LOG(LogTemp, Log, TEXT("Chosen Vector: %s"), *CurrentVector.CurrentVector.ToString());
        ...
        if(!Worldz->LineTraceSingleByChannel(TestObstacles, CurrentVector.CurrentVector, Destination, ECC_Visibility, QueryParams)){
        ...
        for(int32 i = 0; i < Neighbors.Num(); i++){
            ...
            if(!Worldz->LineTraceSingleByChannel(TestObstacles, Neighbors.Last(i).CurrentVector, Destination, ECC_Visibility, QueryParams) && !Neighbors.Last(i).bDidCollide){
        ...
        if(bPathFound){
            ...
            //Make Temp the Neighbor that found the clear ray trace
            FVectorValues* Temp; //= &Neighbors.Last(IndexOfPath);

            //Set the temps(temp) previous FVectorValues* to the CurrentVector that made the neighbor
            // Temp->PreviousPoint = &CurrentVector;

            //We add the Neighbor to the path
            VectorPath.Add(&Neighbors.Last(IndexOfPath));

            //We set the temp(Neighbor) variable to true so we know they have a neighbor
            //Temp->bHasPrevious = true;

            //We Then set the Temp to the current Vector
            Temp = &CurrentVector;
            UE_LOG(LogTemp, Warning, TEXT("First vector: %s"), *Temp->CurrentVector.ToString());
            UE_LOG(LogTemp, Warning, TEXT("Third vector: %s"), *Temp->PreviousPoint->PreviousPoint->CurrentVector.ToString());
            int32 Check = CrashPrevention;
            FVectorValues* TempTwo;
            do{

                VectorPath.Add(Temp);
                TempTwo = Temp->PreviousPoint;
                //UE_LOG(LogTemp, Log, TEXT("Beep"));
                UE_LOG(LogTemp, Warning, TEXT("Vector: %s"), *Temp->PreviousPoint->CurrentVector.ToString());
                UE_LOG(LogTemp, Warning, TEXT("Vector: %s"), *TempTwo->PreviousPoint->CurrentVector.ToString());
                UE_LOG(LogTemp, Warning, TEXT("%i"), Check);
                Temp = TempTwo;
                Check--;

            }while(TempTwo->bHasPrevious && Check != 0);
            for(int32 i = 1; i < VectorPath.Num(); i++){

                DrawDebugLine(Worldz, VectorPath.Last(i)->CurrentVector, VectorPath.Last(i-1)->CurrentVector, FColor::Green, true, 10.0f, 20, 10.0f);

            }
            DrawDebugLine(Worldz, Neighbors.Last(IndexOfPath).CurrentVector, Destination, FColor::Green, true, 10.0f, 15, 10.0f);
        ...
        for(int32 i = 0; i <Neighbors.Num(); i++){
            ...
            if(!ClosedVectors.Contains(CurrentVectorValue) && !CurrentVectorValue.bDidCollide){
                ...
                if(OpenVectors.Contains(CurrentVectorValue)){
                    if(Tempg < CurrentVectorValue.g){
                        for(int32 k = 0; k < OpenVectors.Num(); k++){
                            ...
                            if(OpenVectors.Last(i) == CurrentVectorValue){

                                OpenVectors.Last(i).PreviousPoint = &CurrentVector;
                                OpenVectors.Last(i).bHasPrevious = true;
                                OpenVectors.Last(i).g = Tempg;
                                UE_LOG(LogTemp, Log, TEXT("Previous Vector: %s"), *OpenVectors.Last(i).PreviousPoint->CurrentVector.ToString());
                    ...
                    CurrentVectorValue.PreviousPoint = &CurrentVector;
                    CurrentVectorValue.bHasPrevious = true;
                    UE_LOG(LogTemp, Log, TEXT("Current Vector: %s"), *CurrentVectorValue.CurrentVector.ToString());
                    UE_LOG(LogTemp, Log, TEXT("Previous Vector: %s"), *CurrentVectorValue.PreviousPoint->CurrentVector.ToString());
                ...
                int32 IndexOfChangingVector = 0;
                //UE_LOG(LogTemp, Warning, TEXT("Ref 019"));
